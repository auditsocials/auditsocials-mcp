...
server.tool("glossary_define", "Define a social media advertising / platform-policy compliance term from the AuditSocials Compliance Glossary. Accepts a term name or slug (e.g. 'account suspension' or 'account-suspension'). Returns the full definition, category, applicable platforms, and related terms.", { term: z.string().describe("The term name or slug to define") }, async ({ term }) => {
    const found = findTerm(term);
    if (!found) {
        const suggestions = TERMS.filter((t) => t.term.toLowerCase().includes(term.trim().toLowerCase()))
            .slice(0, 5)
            .map((t) => `- ${t.term} (${t.slug})`);
        const msg = `No exact match for "${term}" in the AuditSocials Compliance Glossary.` +
            (suggestions.length
                ? `\n\nDid you mean:\n${suggestions.join("\n")}`
...
server.tool("glossary_search", "Search the AuditSocials Compliance Glossary by keyword and/or category. Returns matching terms with their short definitions. Use this to discover terms before calling glossary_define.", {
    query: z
        .string()
    ...
    category: z
        .string()
        .optional()
        .describe("Optional category filter: Platform Policy, Legal & Regulatory, Ad Tech & Metrics, Content & Creative, Privacy & Data, Industry-Specific, Enforcement & Risk"),
    limit: z
        .number()
        .int()
        .min(1)
...
server.tool("glossary_list_categories", "List the AuditSocials Compliance Glossary categories with the number of terms in each. Useful to understand the scope of the vocabulary.", {}, async () => {
...
async function main() {
    ...
    console.error(`auditsocials-mcp running (stdio) — ${META.termCount} glossary terms, ${META.license}`);
}
main().catch((err) => {
